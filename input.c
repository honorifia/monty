...
/* Only include this version of getline() if the POSIX version isn't available. */
...
{
  const size_t INITALLOC = 256;
  const size_t ALLOCSTEP = 256;
  size_t num_read = 0;
...

  {
    ...
    while (EOF != (c = getc(fin)))
    {
      ...
      if (num_read >= *pn)
      {
        ...
        char * tmp = _realloc(*pline_buf, ALLOCSTEP, n_realloc + 1); /* +1 for the trailing NUL. */
        if (NULL != tmp)
        {
